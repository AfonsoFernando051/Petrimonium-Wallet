...
void main() {
  ...
  Widget buildTestable() {
    return MaterialApp(
      theme: AppTheme.dark,
      home: AcademyDomainDetailScreen(domain: testDomain, mascotController: mascotController),
    );
  }

  Future<void> pumpUntilLoaded(WidgetTester tester) async {
    // Never pumpAndSettle here — CosmicBackground's AnimationControllers
    // repeat indefinitely. Several plain pumps flush the controller's async
    // load() chain (progress load -> catalog cache -> remote merge).
    for (var i = 0; i < 8; i++) {
      await tester.pump();
    }
  }

  group('AcademyDomainDetailScreen', () {
    testWidgets('renders the domain description and its schools once loaded', (tester) async {
      await tester.pumpWidget(buildTestable());
      await pumpUntilLoaded(tester);

      expect(find.text(testDomain.description), findsWidgets);
      expect(find.text(testSchool.title), findsOneWidget);
      expect(find.text(testComingSoonSchool.title), findsOneWidget);
    });

    testWidgets('navigates to SchoolDetailScreen when an available school is tapped', (tester) async {
      await tester.pumpWidget(buildTestable());
      await pumpUntilLoaded(tester);

      await tester.tap(find.text(testSchool.title));
      await pumpUntilLoaded(tester);

      expect(find.byType(SchoolDetailScreen), findsOneWidget);
    });
  });
}
